...
- 既定は単一エージェントで直接実行する。機械的に検証できる修正の反復で収束するならLoop、独立した調査やレビューの隔離で足りるならSubagent、複数workstreamの分岐・合流・選択的再実行が必要なときだけGraphを使う。判断基準は`.agents/skills/loop-engineering/SKILL.md`にある。
- 自律的な修復は最大3回。同じ根本原因が新しい根拠なしに2回続いたら止める。
- 止めるときは、観測した失敗・推定した根本原因・試したこと・根拠・推奨する次の手を報告する。
- SkillやMCPを先回りして読み込まず、必要なときに必要なものだけ読む。大きな調査結果は要約してから先へ進む。
